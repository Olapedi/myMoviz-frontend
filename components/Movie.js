...
function Movie(props) {
  ...
  return (
    <div className={styles.card}>
      <img className={styles.image} src={`https://www.themoviedb.org/t/p/w600_and_h900_bestv2${props.poster}`} alt={props.title} />
      <div className={styles.textContainer}>
        <div>
          <span className={styles.name}>{props.title}</span>
          <p className={styles.description}>{props.overview.length > 250 ? `${props.overview.substring(0, 249)}...` : props.overview}</p>
        </div>
